...
Features present in the legacy Node.js server (`legacy-nodejs/`) that are not
yet implemented in the Deno rewrite. Work through these one at a time to reach
full drop-in replacement parity.
...
- New: implement a `src/workers/prune-worker.ts` (or a `Deno.cron` /
  `setInterval` loop on the main thread) that calls a
  `pruneExpired(db, storage, rules)` function in `src/domain/prune.ts`.
...
`src/storage/s3.ts` and the config schema are already stubbed. `main.ts`
currently calls `Deno.exit(1)` when `storage.backend === "s3"`.
...
- `read` → `GetObject` response body stream (or redirect to `publicURL` if
  configured).
- `has` / `size` → `HeadObject`.
- `remove` → `DeleteObject`.
...
- **Rebuild** the admin SPA as a Hono JSX component (larger effort, lower
  priority).
...
configured list of upstream CDN domains. First 2xx response with a
`Content-Length` wins — fetch and stream to client, optionally cache.

- Add `discovery.upstream.enabled` and `discovery.upstream.domains` to config
  schema.
...
video), stores the _output_ blob, and returns its descriptor. The SHA-256 in the
auth event must match the _input_ file.

- Add `media.*` config keys (quality, maxWidth, maxHeight, outputFormat, codecs,
  etc.).
