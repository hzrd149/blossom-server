...
export class UploadForm extends LitElement {
  ...
  render() {
    const preview = !this.selected
      ...
      `
      : html`
        <p class="pointer-none text-gray-500 font-bold">${this.selected
          .name}</p>
        <p class="text-gray-500 text-sm">${formatBytes(this.selected.size)}</p>
      `;

    return html`
      <div class="w-full px-10 pt-10 pb-6 bg-white rounded-xl flex flex-col">
        ...
        <form class="space-y-3" @submit="${this.upload}">
          <div class="grid grid-cols-1 space-y-2">
            ...
            <div class="flex items-center justify-center w-full">
              ...
              >
                ${preview}
                <input name="blob" type="file" class="hidden" @change="${this
                  .inputChange}" />
              </label>
            </div>
        ...
        <div class="flex gap-4 mt-2">
          ...
          ${window.nostr && html`
            <a class="text-md text-blue-500" href="#mirror">Mirror Blobs</a>
          `}
          <!-- <a class="text-md text-red-500" href="#delete">Request delete</a> -->
        </div>
        <a
          class="text-sm text-blue-400 ml-auto mt-4"
