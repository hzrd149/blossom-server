...
export class ListForm extends LitElement {
  ...
  async submit(e) {
    ...
    this.pubkey = formData.get("pubkey") ||
    ...
    this._auth = await window.nostr.signEvent({
      ...
      tags: [
        ...
        [
          "server",
          new URL(location.protocol + "//" + location.hostname).toString(),
        ],
      ],
    });
              ...
              html`
                <tr class="whitespace-nowrap">
                  <td class="px-2 overflow-hidden truncate font-mono" style="max-width: 10em">
                    <a href="${blob
                      .url}" class="hover:underline" target="_blank"
                    >${blob.sha256}</a>
                  </td>
                  <td class="px-2 text-right" style="max-width: 5em">${blob
                    .type}</td>
  ...
  renderForm() {
    return html`
      <form class="space-y-4 ml-auto" @submit="${this.submit}">
        <label class="text-sm font-bold text-gray-500 tracking-wide flex flex-col">
          ...
          ${window.nostr &&
            html`
              <a href="#" class="text-blue-500 ml-auto" @click="${this
                .pubkeyFromExtension}"
              >From Extension</a>
            `}
        </label>
  ...
  renderContent() {
    if (this.status) {
      return html`
        <p class="my-5 text-center text-lg">${this.status}</p>
      `;
    } else if (this.blobs) {
      if (this.blobs.length === 0) {
        return html`
          <p class="text-gray-500 text-md text-center p-10">Set pubkey</p>
        `;
      }
  ...
  render() {
    ...
    return html`
      <div class="w-full p-4 bg-white rounded-xl flex flex-col overflow-hidden">
