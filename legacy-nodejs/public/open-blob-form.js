...
export class GetBlobForm extends LitElement {
  ...
  async submit(e) {
    ...
    this.hasBlob = await fetch("/" + sha256, { method: "HEAD" }).then((res) =>
      res.ok
    );
  ...
  renderResults() {
    return html`
      <div class="flex gap-2 flex-col max-h-xl overflow-auto">
        ${this.blobs.map(
          (blob) =>
            html`
              <div class="flex gap-2">
                <a href="${blob
                  .url}" class="hover:underline" target="_blank">${blob
                  .sha256}</a>
              </div>
            `,
        )}
  ...
  renderForm() {
    return html`
      <form class="gap-2" @submit="${this.submit}">
        <label class="text-sm font-bold text-gray-500 tracking-wide flex flex-col">
          ...
          <div class="flex gap-2">
            <input
              name="sha256"
              type="text"
              class="rounded-md border-2 w-full p-2 h-10"
              required
            />
            <button
              type="submit"
        ...
        ${this.hasBlob === false
          ? html`
            <p class="text-red-500 mb-4">Blob missing</p>
          `
          : null}
      </form>
    `;
