...
export const BlobsPage: FC<BlobsPageProps> = async ({ db, config, host, page, q }) => {
  const offset = (page - 1) * PAGE_SIZE;
  const filter = q ? { q } : undefined;

  const [blobs, total] = await Promise.all([
    db.listAllBlobs({ filter, limit: PAGE_SIZE, offset, sort: ["uploaded", "DESC"] }),
  ...
  return (
    <AdminLayout title="Blobs" section="blobs">
      <PageHeader title="Blobs" subtitle={`${total.toLocaleString()} total blob${total !== 1 ? "s" : ""}`} />
      ...
      {blobs.length === 0 ? (
        <EmptyState message={q ? `No blobs matching "${q}"` : "No blobs stored yet."} />
      ) : (
        <>
          <Table>
            <Thead>
            ...
            <Tbody>
              {blobs.map((blob) => {
                ...
                return (
                  <tr key={blob.sha256} class="hover:bg-gray-900 transition-colors">
                    <Td mono>
                      <a
                    ...
                    <Td>
                      {blob.type ? (
                        <Badge color={mimeColor(blob.type)}>{blob.type}</Badge>
                      ) : (
                    ...
                    <Td>
                      <div class="flex gap-2 items-center">
                        ...
                        <DangerButton
                          onclick={`adminAction('${deleteUrl}','DELETE','Delete blob ${truncateHash(blob.sha256)}? This cannot be undone.')`}
                        >
                          Delete
                        </DangerButton>
                      </div>
