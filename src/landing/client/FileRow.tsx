...
export function FileRow({
  ...
  const showProgress = uf.status === "uploading" || uf.status === "retrying" ||
    uf.status === "done";
  const pct = uf.status === "done" ? 100 : uf.progress;
  const hasResultUrl = (uf.status === "done" || uf.status === "exists") &&
    uf.result;
    ...
    <div class="bg-gray-800 rounded-lg px-4 py-3 space-y-2 min-w-0">
      ...
      {uf.status !== "pending" && (
        <div class="flex items-center gap-2 min-w-0">
          ...
          {uf.status === "skipped" && uf.error && (
            <span class="flex-1 text-xs text-amber-400 min-w-0 break-words">
              {uf.error}
            </span>
          )}
